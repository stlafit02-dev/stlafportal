...
export function EmailAccountPage() {
  ...
  const [recycleAccount, setRecycleAccount] = useState<EmailAccount | null>(null);
  ...
  async function loadAll() {
    setIsLoading(true);
    const [gws, emails] = await Promise.all([fetchGwsAccounts(), fetchEmailAccounts()]);
    setGwsAccounts(gws);
  ...
  function handleSaved(account: EmailAccount) {
    setEmailAccounts((prev) => prev.map((e) => (e.id === account.id ? account : e)));
  }
    ...
    <div className="gmail-page">
      <div className="email-toolbar-header">
        <div>
          ...
          <p className="page-subtitle">{emailAccounts.length} records · Click row to view details</p>
        </div>
        <button className="gmail-primary-btn" onClick={() => setIsAddModalOpen(true)}>
          + Email Account
        </button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="gmail-empty">
          {emailAccounts.length === 0 ? "No email accounts yet." : "No accounts match your search or filter."}
        </div>
      ) : (
        <div className="gmail-table-wrap email-table-wrap">
          <table className="gmail-table">
            ...
            <tbody>
              {filtered.map((e) => (
                <tr key={e.id} className="email-row" onClick={() => handleRowClick(e)}>
                  <td>{e.fullName}</td>
                  <td className="mono-cell">{e.localGmail}</td>
                  <td className="mono-cell">{e.stlafEmail}</td>
                  <td>{e.gwsAccountName}</td>
                  <td>
                    <span className={`status-badge ${e.status === "Active" ? "badge-active" : "badge-inactive"}`}>
                      {e.status}
                    </span>
                  </td>
                  <td>{e.oldUser || <span className="unassigned-text">—</span>}</td>
                  <td className="email-date-cell">
                    {new Date(e.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                  </td>
                  <td onClick={(evt) => evt.stopPropagation()}>
                    <div className="action-icons">
                      <button className="icon-btn" onClick={() => setEditAccount(e)} aria-label="Edit account">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          <path d="M17 3a2.83 2.83 0 0 1 4 4L7.5 20.5 2 22l1.5-5.5Z" />
                        </svg>
                      </button>
                      <button className="icon-btn icon-btn-danger" onClick={() => setPendingDelete(e)} aria-label="Delete account">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <EmailAccountDetailModal account={viewAccount} onClose={() => setViewAccount(null)} />
      ...
      <ConfirmDialog
        ...
        message={pendingDelete ? `Remove ${pendingDelete.stlafEmail} — ${pendingDelete.fullName}? This cannot be undone.` : ""}
        confirmLabel="Remove"
        danger
