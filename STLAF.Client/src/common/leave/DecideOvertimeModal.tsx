...
import { decideDeptOvertime, decidePartnerOvertime, type OvertimeRequest } from "./overtimeApi";
import "../../departments/it/gmail/GmailForms.css";

interface DecideOvertimeModalProps {
  request: OvertimeRequest | null;
  approved: boolean;
  stage: "dept" | "partner";
  onClose: () => void;
  onDecided: (request: OvertimeRequest) => void;
}

export function DecideOvertimeModal({ request, approved, stage, onClose, onDecided }: DecideOvertimeModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const decide = stage === "dept" ? decideDeptOvertime : decidePartnerOvertime;
      const updated = await decide(current.id, approved, notes || undefined);
      onDecided(updated);
      onClose();
      setNotes("");
    } catch {
    ...
    <Modal isOpen={!!request} onClose={onClose}>
      <div className="gmail-modal">
        <h2 className="gmail-modal-title">
          {approved ? "Approve" : "Reject"} Overtime{stage === "partner" ? " (Final)" : ""}
        </h2>
